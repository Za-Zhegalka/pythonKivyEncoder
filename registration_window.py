...
import bcrypt
import re
import dns.resolver  # Импортируем библиотеку для DNS-запросов
...
class RegistrationWindow(BoxLayout):
    ...
    def register(self, instance):
        ...
        try:
            cursor.execute(
                "INSERT INTO users (username, password_hash, keyword_hash, email) VALUES (%s, %s, %s, %s)",
                (username, password_hash, keyword_hash, email)
            )
            db.commit()
